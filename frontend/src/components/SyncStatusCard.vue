<template>
  ...
  <button class="text-sm text-slate-500" :disabled="loadingStatus" @click="handleRefresh">
        ...
        <button
          v-if="isAdmin"
          class="text-sm rounded bg-orange-500 px-3 py-1 text-white"
    ...
    <section class="grid gap-4 md:grid-cols-3">
      ...
      <article class="rounded border p-3">
        <p class="text-xs uppercase tracking-wide text-slate-500">今日成功率（运行状态）</p>
...
onMounted(() => {
  syncStore.fetchStatus();
  if (isAdmin.value) syncStore.fetchConflicts();
});

watch(isAdmin, (value) => {
  if (value) syncStore.fetchConflicts();
});
</script>
